...
class GameMap:
    ...
    def place_entities(self, room, entities, max_monsters_per_room):
        ...
        for i in range(number_of_monsters):     #Place the monster in a random place in the room
            x = randint(room.x1 + 1, room.x2 - 1)
            y = randint(room.y1 + 1, room.y2 - 1)
        
            if not any([entity for entity in entities if entity.x == x and entity.y == y]): #Hard-coded display of 2 possible monsters. Cahnge later.
                if randint(0, 100) < 80:
